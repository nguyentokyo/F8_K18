...
function getMaxTotalPrice(arr) {
    ...
    let maxTotalPrice = arr[0].totalPrice;
    let name = arr[0].name;

    for (let i = 0; i < arr.length; i++) {
        if (maxTotalPrice < arr[i].totalPrice) {
            maxTotalPrice = arr[i].totalPrice;
            name = arr[i].name;
        }
    }
    return {name , maxTotalPrice};
}
console.log('---result---');
console.log('Product Name :', getMaxTotalPrice(products).name);
console.log('Total Price :', getMaxTotalPrice(products).maxTotalPrice);
